...
define('PATH', 'public://template-proxy-images');//relative folder where the images are saved
...
function setHeaders($nocache) {
  if ($nocache === FALSE && is_int(CCACHE) && CCACHE > 0) {
    ...
    drupal_add_http_header('Last-Modified', GMDATECACHE . ' GMT');
    drupal_add_http_header('Cache-Control', 'max-age=' . (CCACHE - 1));
    drupal_add_http_header('Pragma', 'max-age=' . (CCACHE - 1));
    drupal_add_http_header('Expires', gmdate('D, d M Y H:i:s', INIT_EXEC + CCACHE - 1));
    drupal_add_http_header('Access-Control-Max-Age', CCACHE);
  }
  else {
    //no-cache
    drupal_add_http_header('Pragma', 'no-cache');
    drupal_add_http_header('Cache-Control', 'no-cache');
    drupal_add_http_header('Expires', GMDATECACHE . ' GMT');
  }

  //set access-control
  drupal_add_http_header('Access-Control-Allow-Origin', '*');
  drupal_add_http_header('Access-Control-Request-Method', '*');
  drupal_add_http_header('Access-Control-Allow-Methods', 'OPTIONS, GET');
  drupal_add_http_header('Access-Control-Allow-Headers', '*');
}
