 ...
 */
function mdstructure_default_install_tasks(&$install_state) {

  $tasks = array();

  // Add our custom CSS file for the installation process
  drupal_add_css(drupal_get_path('profile', 'mdstructure_default') . '/mdtheme.css');
 ...
 */
function mdstructure_default_form_install_configure_form_alter(&$form, $form_state) {
  $form['site_information']['site_name']['#default_value'] = 'MD Structure Default';
  $form['#submit'][] = 'mdstructure_default_install_configure_form_submit';
}

/**
 * Forms API submit for the site configuration form.
 */
function mdstructure_default_install_configure_form_submit($form, &$form_state) {
  global $user;
  $sql_file = dirname(__FILE__) . '/mdstructure_default.sql';
  $count = mdstructure_default_import_sql($sql_file);
...
function mdstructure_default_import_sql($filename) {
  global $databases;
  $databasesName = array(
    "pgsql" => "PostgreSQL",
    "sqlite" => "SQLite"
  ...
  if ($file) {
    ...
    if ($connect) {
      while (!feof($file)) {
        ...
        if (preg_match('|;$|', $line)) {
          try {
            $connect->exec(mdstructure_default_prefixTables($sql_command));
...
function mdstructure_default_prefixTables($sql) {
  global $databases;
